      ...
      {items.map((item, i) => (
        <div key={i} className="border border-surface-200 rounded-xl overflow-hidden">
          ...
          >
            ...
            <ChevronDown className={`w-5 h-5 shrink-0 text-ink-500 transition-transform duration-300 ${open === i ? "rotate-180" : ""}`} />
          </button>
          {open === i && (
            <div className="px-6 pb-4 text-ink-700 leading-relaxed">{item.a}</div>
          )}
        </div>
      ))}
    </div>
